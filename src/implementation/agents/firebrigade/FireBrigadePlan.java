...
public class FireBrigadePlan extends AbstractPlan {
    ...
    public Object createPlan(Beliefs beliefs, Desires desires) {
        ...
        int distance = ((LocationBelief) beliefs.getBelief(BeliefType.REPAIR_DISTANCE)).getEntityID().getValue();//aquí está bien repair distance?
        ...
        if (target != null) {
            ...
            EntityMapBelief entityMapBelief = (EntityMapBelief)beliefs.getBelief(BeliefType.BUILDINGS_ON_FIRE); //Changed to BUildings on fire.
            ...
            getAgent().sendExtinguish(time, target.getID(), tmpWaterLevel); //Aquí se manda el send Extinguish. o similar.
            ...
            //If it does not have a Goal location in its desires.
            if (goalLocation == null) {
                ...
                //Then it has a goal location desire.
                EntityID position = ((Human) getAgent().me()).getPosition();

                //If already at the goal location.
                if (goalLocation.getEntityID().getValue() == position.getValue()) {
                    ...
                    //If it does not have more next goals.

                        //If it does not have any queued messages which could contain a new Goal location.
                        if(getAgent().getQueuedMessages().size() <= 0){

                            System.out.println("SOLICITAR DONDE APAGAR INCENDIO.");

                            int conversationId = getAgent().nextConversationId();

                            ACLMessage requestFire = new ACLMessage(time,getAgent().getID(), ACLPerformative.REQUEST,new EntityID(0),
                                    conversationId, ActionConstants.REQUEST_FIRE,0,0,0,0,new EntityID(0),0);

                            getAgent().addACLMessageToQueue(conversationId,requestFire);

                            getAgent().addACLMessage(requestFire);
                        }
